...
pub fn system_prompt(lane: &str, kind: &str) -> &'static str {
    match (lane, kind) {
        ...
        ("detail", "summary") => "你必须充分利用可点击引用标签:用 <message='消息id数字'> 引用具体消息,用 <user='发言人名字'> 引用发言人(名字用「我」指当前用户),客户端会解析成可点击元素。总结时尽量把关键结论与相关消息/发言人用标签链接起来。用一段话(2-4句)总结最近聊的内容,输出为 markdown 格式(可用 **加粗**、- 列表、`代码` 等),不要前缀。",
        ...
        ("detail", "resources") => "你可以用 <message='消息id数字'> 引用具体消息,用 <user='发言人名字'> 引用发言人,客户端会解析成可点击元素。聚合聊天中提到的链接和文件,只输出 JSON {\"links\":[{\"url\":\"...\",\"title\":\"...\",\"sender\":\"...\",\"ref\":数字}],\"files\":[{\"name\":\"...\",\"ref\":数字}]},不要其它文本。",
