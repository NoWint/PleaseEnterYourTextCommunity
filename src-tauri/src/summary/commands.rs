...
pub fn system_prompt(lane: &str, kind: &str) -> &'static str {
    match (lane, kind) {
        ...
        ("detail", "summary") => "你必须充分利用可点击引用标签:用 <message='消息id数字'> 引用具体消息,用 <user='发言人名字'> 引用发言人(名字用「我」指当前用户),客户端会解析成可点击元素。总结时尽量把关键结论与相关消息/发言人用标签链接起来。用一段话(2-4句)总结最近聊的内容,输出为 markdown 格式(可用 **加粗**、- 列表、`代码` 等),不要前缀。引用时务必原样输出消息 id 与发言人名字,可加 <time='14:30'> 标注关键时间点。",
        ("detail", "mood") => "分析聊天记录的情绪氛围,只输出 JSON 对象 {\"overall\":\"积极|中立|消极|混合\",\"score\":0-100,\"emoji\":\"🔥\",\"summary\":\"一段话解读(2-4句)\",\"highlights\":[{\"text\":\"代表消息摘要\",\"emoji\":\"😄\"}]},不要输出任何其它文本。",
        ("detail", "participation") => "你必须充分利用可点击引用标签:用 <message='消息id数字'> 引用具体消息,用 <user='发言人名字'> 引用发言人(名字用「我」指当前用户),客户端会解析成可点击元素。根据提供的参与度统计(成员消息数/字符数/活跃天数、时段分布、每日密度),用一段 markdown 文本解读参与情况,尽量用标签指向代表消息与发言人,突出最活跃者与活跃时段。",
        ("detail", "action_items") => "你可以用 <message='消息id数字'> 引用具体消息,用 <user='发言人名字'> 引用发言人,客户端会解析成可点击元素。提取聊天中的行动项/待办事项,只输出 JSON 对象 {\"items\":[{\"text\":\"...\",\"assignee\":\"...\",\"due\":\"...\",\"ref\":数字}]},不要输出任何其它文本。",
        ("detail", "open_questions") => "你可以用 <message='消息id数字'> 引用具体消息,用 <user='发言人名字'> 引用发言人,客户端会解析成可点击元素。找出聊天中悬而未决的问题(提问后无人明确回答),只输出 JSON {\"questions\":[{\"text\":\"...\",\"asked_by\":\"...\",\"ref\":数字}]},不要其它文本。",
        ("detail", "timeline") => "你可以用 <message='消息id数字'> 引用具体消息,用 <user='发言人名字'> 引用发言人,客户端会解析成可点击元素。把聊天按话题演变划分为若干阶段,只输出 JSON {\"phases\":[{\"period\":\"...\",\"topic\":\"...\",\"key_messages\":[数字]}]},不要其它文本。",
        ("detail", "decisions") => "你可以用 <message='消息id数字'> 引用具体消息,用 <user='发言人名字'> 引用发言人,客户端会解析成可点击元素。提取聊天中做出的决策及理由,只输出 JSON {\"decisions\":[{\"title\":\"...\",\"by\":\"...\",\"rationale\":\"...\",\"ref\":数字}]},不要其它文本。",
