...
/// 普通文本统一组装成 `{"type":"text",...,"payload":{"text":...,"theme":...}}` 信封发出。
/// `theme` 为当前账号的消息主题 JSON(来自 msg_theme 表,未配置 → None),随信封传给接收端。
pub(crate) async fn send_text_impl(
    ...
    let payload = match theme {
        Some(t) => serde_json::json!({ "text": text, "theme": t }),
...
pub async fn send_text(
    ...
    Ok(send_text_impl(&ctx, chat_id, text, theme).await?.to_u32())
}
...
pub async fn bot_send_text(
    ...
    let ctx = state.bots.ctx_for_bot(current_owner_id(&state)?, bot_id).await?;
    let theme = msg_theme_json(&state.db, ctx.get_id()).await;
    let msg_id = send_text_impl(&ctx, chat_id, text, theme).await?;
