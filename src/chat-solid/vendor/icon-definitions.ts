/**
 * Vendored icon definitions from opencode desktop v2 (`packages/ui/src/v2/components/icon.tsx`).
 *
 * Pure data only: an `icons` map of name → { viewBox, body }. No Solid imports,
 * no JSX, no DOM code. The original file also ships an `ensureSprite()` helper
 * and a Solid `<Icon>` component; those are intentionally NOT included here so
 * this module stays framework-agnostic. Downstream code that needs the sprite
 * or component should build them on top of this data.
 */
