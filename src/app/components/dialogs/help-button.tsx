...
// IM 版 = 标题栏问号按钮，弹出帮助对话框（使用指南/反馈/关于）。
// 使用指南/反馈经 platform.openExternal 打开真实链接（Tauri openUrl / 浏览器新窗口）。
...
/** 文档/反馈入口（应用品牌域名，供邀请链接共用；文档页由服务端维护）。 */
export const HELP_DOCS_URL = "https://peyt.yzjtiantian.cn"
export const HELP_FEEDBACK_URL = "https://peyt.yzjtiantian.cn/feedback"
      ...
      <DialogBody class="flex flex-col gap-6 px-6 pb-6">
        ...
        <div class="flex flex-col gap-4">
          <HelpItem
            ...
            onClick={() => platform.openExternal(HELP_DOCS_URL)}
          />
          <HelpItem
            title={dialogsT("help.drawer.feedback")}
            description={dialogsT("help.drawer.feedback.description")}
            onClick={() => platform.openExternal(HELP_FEEDBACK_URL)}
          />
          <HelpItem
...
function HelpItem(props: { title: string; description: string; onClick?: () => void }) {
  const content = (
    <>
  ...
  if (!props.onClick) {
    return <div class="flex flex-col gap-1">{content}</div>
  }
  return (
    <button
      type="button"
      class="flex flex-col gap-1 rounded-md px-1 -mx-1 py-1 text-start transition-colors hover:bg-v2-overlay-simple-overlay-hover"
