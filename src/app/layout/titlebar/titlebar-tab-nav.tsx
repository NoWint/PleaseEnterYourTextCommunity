// src/app/layout/titlebar/titlebar-tab-nav.tsx
// 照抄 opencode components/titlebar-tab-nav.tsx 改造：
// - @tanstack/solid-query 的 createMutation → 本地 pending 信号
// - server/global 依赖 → 本地 layout.projects / 会话直取
// - 保留：内联重命名、双击改名、中键关闭、预览 popover
...
export function TabNavItem(props: {
  ...
  const project = createMemo(() => {
    ...
    if (!session) return
    return layout.projects.list().find((item) => item.worktree === session.directory)
  })
  ...
  const openRename = (event: MouseEvent) => {
    event.preventDefault()
    event.stopPropagation()
    if (!canOpenTabRename(props.dragging, editing(), pending())) return
    const session = props.session()
    if (!session) return
