...
| **[frontend.md](frontend.md)** | 前端 46 个 TS 文件的地图：启动、状态/类型、持久化、路由、页面、聊天、卡片、组件、插件前端、主题、动效、模块依赖 | 改前端任何功能 |
| **[backend.md](backend.md)** | 后端 10 个 Rust 文件：启动、AppState、79 个命令分组、卡片命令详解、插件后端、终端、错误处理、配置 | 改 Tauri 命令 / 后端逻辑 |
...
│   └── views/login.ts         # 登录页
└── src-tauri/                 # 后端（10 个 Rust 文件）
    ├── src/
    │   ├── main.rs            # 入口 → peytchat_lib::run()
    │   ├── lib.rs             # Tauri builder，注册全部 79 个命令 + 事件转发
    │   ├── state.rs           # AppState（accounts / db / plugins / terminals）
    │   ├── commands.rs        # 全部业务命令（~2114 行）
