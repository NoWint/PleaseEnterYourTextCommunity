...
| | 保存消息 / 设备聊天 | 无 | 低 | ✅ 完成 |
| | 草稿系统 | 无 | 低 | ✅ 完成 |
| **2 消息体验** | 全局搜索 + 会话内搜索 | 批次1 | 中 | ✅ 完成 |
| | Gallery 相册 | 批次1 | 中 | ✅ 完成 |
| | 命令面板 | 批次1 | 中 | ✅ 完成 |
| | 邮件列表 / 广播列表 | 无 | 低 | ✅ 完成 |
| **3 媒体与运行时** | 语音消息 | 批次2 | 高 | ✅ 完成 |
| | Webxdc 应用 | 批次2 | 高 | ✅ 完成 |
| **4 系统与安全** | 系统通知弹窗回复 | 批次3 | 中 | ✅ 完成 |
| | 验证群 / 保护状态 | 批次3 | 中 | ✅ 完成 |
| | 多设备绑定 | 批次3 | 高 | ✅ 完成 |
| | 备份 / 恢复 | 批次3 | 中 | ✅ 完成 |
| | **通话**（VoIP 音频/视频） | 批次3 | 高 | ⏳ 待做 |
...
### 批次 1 验收 ✅ 已完成
- [x] 会话可归档/取消归档，归档视图可切换
- [x] 消息可保存/取消保存，「保存的消息」入口可访问
- [x] Composer 草稿在会话切换间保留，列表显示草稿标记

### 批次 2 验收 ✅ 已完成
- [x] 全局搜索返回会话/联系人/消息，可跳转高亮
- [x] Gallery 网格 + 全屏媒体查看可用
- [x] Cmd+P 命令面板可执行主要命令
- [x] 广播/邮件列表会话正确识别并显示已读计数

### 批次 3 验收 ✅ 已完成
- [x] 可录制并发送语音消息，消息内播放器播放/暂停 + 计时
- [x] webxdc 应用卡片可打开运行，StatusUpdate 实时同步
...
- 2026-08-03 实施进度：
  - **批次 1 完成**：归档、保存消息、草稿（plan `2026-08-03-delta-batch1-archive-saved-draft.md`）。
  - **批次 2 完成**：全局搜索+会话内搜索、Gallery、命令面板、邮件/广播列表（plan `2026-08-03-delta-batch2-search-gallery-palette-mailing.md`）。
  - **批次 3 完成**：语音消息、Webxdc 应用（plan `2026-08-03-delta-batch3-voice-webxdc.md`）。附带代码审查修复（webxdc 沙箱安全、状态同步契约、录音切页泄漏）。
  - **批次 4 完成**：系统通知（队列合并）、保护状态（指纹）、多设备绑定、备份/恢复（plan `2026-08-03-delta-batch4-notifications-encryption-backup.md`）。
  - **额外完成**：Delta 式全量 DOM 消息渲染重构（替代手写虚拟化，根治滚动闪烁/微动）、单聊 UI（头部只显对方用户名、气泡内不显 name/role）、字体放大（Apple 规范）。
